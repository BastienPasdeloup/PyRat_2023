    ...
    #############################################################################################################################################
        ...
        def reset (self, seed=None, options=None) :
            ...
            if seed is not None :
                ...
                self.game_random_seed_maze = nprandom.randint(max_rand_value) if seed_maze is None else seed_maze
                self.game_random_seed_cheese = nprandom.randint(max_rand_value) if seed_cheese is None else seed_cheese
                self.game_random_seed_players = nprandom.randint(max_rand_value) if seed_players is None else seed_players
        ...
        def _render_gui (self) :
            ...
            def pygame_thread () :
                try :
                    ...
                    current_team_scores = self._score_per_team()
                    ...
                    while running :
                        ...
                        try :
                            ...
                            current_team_scores = team_scores
            ...
            if self.turn == 0 :
                self.gui_process = multiprocessing.Process(target=pygame_thread)
    ...
    #############################################################################################################################################
        ...
        def start (self, reset=False) :
            ...
            try :
                ...
                turn_start_synchronizer = multiprocessing.Barrier(len(self.player_locations) + 1)
                turn_timeout_lock = multiprocessing.Lock()
                player_processes = {}
                for player in self.player_locations :
                    def player_wrapper (input_queue, output_queue, turn_end_synchronizer) :
                        try :
                            while True :
                    ...
                    player_processes[player]["process"] = multiprocessing.Process(target=player_wrapper, args=(player_processes[player]["input_queue"], player_processes[player]["output_queue"], player_processes[player]["turn_end_synchronizer"],))
                ...
                if not self.synchronous :
                    ...
                    for player in player_processes :
                        ...
                        waiter_processes[player]["process"] = multiprocessing.Process(target=waiter_thread, args=(waiter_processes[player]["input_queue"],))
